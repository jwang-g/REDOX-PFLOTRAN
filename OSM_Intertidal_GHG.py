...
#Zmar=[0.5*math.sin(slope*math.pi/180),1.5*math.sin(slope*math.pi/180),2.5*math.sin(slope*math.pi/180)]    ##marsh elevation in meters from MLW to MHW for three stations, 0.5 is distance from the middle of the soil colum surface to MLW and the total distance is 3m for three column.
Zmar=[0.5*math.sin(slope*math.pi/180),1.5*math.sin(slope*math.pi/180),2.5*math.sin(slope*math.pi/180)]
##sharp break morphology soil column is 1.5m higher than mean low water level
#Zmar=[1.5,1.5,1.5]   #meso tidal setting
#Zmar=[0,0,0]           #micro tidal setting
...
#cjw 10-yr run set up, change to the following setting for 10-year run including leap year and non-leap year
    repyr=3
    yrchain=[2011]#[2009,2010,2011,2012,2013,2014,2015,2016,2017,2018,2019]
...
#    Zs=0.0      #marsh elevation for MRD
    #print(stnm)
    #Zmar=[0.5*math.sin(slope*math.pi/180),1.5*math.sin(slope*math.pi/180),2.5*math.sin(slope*math.pi/180)]    ##marsh elevation in meters from MLW to MHW for three stations, 0.5 is distance from the middle of the soil colum surface to MLW and the total distance is 3m for three column.
    Zs=Zmar[clnum]
    clnum=clnum+1
