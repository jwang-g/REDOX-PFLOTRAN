...
decomp_network.decomp_pool(name='Fe+++',kind='primary',constraints={'initial':'1e-5'}),
...
reactions = [
    decomp_network.reaction(name='Aerobic decomposition',reactant_pools={'SOM':1.0},product_pools={'HCO3-':1.0},reactiontype='SOMDECOMP',
                                            rate_constant=1e-3,rate_units='y', 
    ...
    decomp_network.reaction(name='Hydrolysis',stoich='1.0 SOM -> 1.0 DOM1',reactiontype='SOMDECOMP',
                                            rate_constant=1e-2,rate_units='y', #  Jianqiu Zheng et al., 2019: One third of fermented C is converted to CO2
    ...
    decomp_network.reaction(name='Nitrification',stoich='2.0 NH4+ + 4.0 O2(aq) -> 2.0 NO3- + 2.0 H2O + 4.0 H+',
                        monod_terms=[decomp_network.monod(species='NH4+',k=conc_scales['NH4+'],threshold=1.1e-15),decomp_network.monod(species='O2(aq)',k=conc_scales['O2(aq)'],threshold=1.1e-15)],
                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
    ...
    decomp_network.reaction(name='Denitrification',stoich='1.0 Acetate- + 2.0 NO3- + 1.0 H+ -> 2.0 HCO3- + 1.0 N2(aq) + 0.0 N2O(aq) + 2.0 H2O + 2.0 Tracer',
                                            monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=1.1e-15),decomp_network.monod(species='NO3-',k=conc_scales['NO3-'],threshold=1.1e-15)],
                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
    ...
    decomp_network.reaction(name='Sulfate reduction',stoich='1.0 Acetate- + 1.0 SO4--  -> 2.0 HCO3- + 2.0 HS- +  2.0 Tracer',
                                            monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=1.1e-15),decomp_network.monod(species='SO4--',k=conc_scales['SO4--'],threshold=1.1e-15)],
                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD'),
                                                                decomp_network.inhibition(species='NO3-',k=conc_scales['NO3-'],type='MONOD'),
                                                                decomp_network.inhibition(species='Fe+++',k=conc_scales['Fe+++'],type='MONOD')],
    ...
    decomp_network.reaction(name='Methane oxidation (O2)',stoich='1.0 CH4(aq)  + 1.0 O2(aq)  -> 1.0 HCO3-  + 1.0 H+ + 1.0 H2O + 1.0 Tracer',
                                            monod_terms=[decomp_network.monod(species='O2(aq)',k=conc_scales['O2(aq)'],threshold=1.1e-12),decomp_network.monod(species='CH4(aq)',k=conc_scales['CH4(aq)'],threshold=1.1e-16)],
                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
    ...
    decomp_network.reaction(name='Methane oxidation (NO3)',stoich='1.0 CH4(aq)  + 1.0 NO3-  -> 1.0 HCO3-  + 1.0 NH4+ + 1.0 Tracer ',
                                            monod_terms=[decomp_network.monod(species='NO3-',k=conc_scales['NO3-'],threshold=1.1e-12),decomp_network.monod(species='CH4(aq)',k=conc_scales['CH4(aq)'],threshold=1.1e-16)],
                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
    ...
    decomp_network.reaction(name='Methane oxidation (SO4)',stoich='1.0 CH4(aq)  + 1.0 SO4-- -> 1.0 HCO3-  + 1.0 HS- + 1.0 H2O + 1.0 Tracer ',
                                            monod_terms=[decomp_network.monod(species='SO4--',k=conc_scales['SO4--'],threshold=1.1e-12),decomp_network.monod(species='CH4(aq)',k=conc_scales['CH4(aq)'],threshold=1.1e-16)],
                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
    ...
    decomp_network.reaction(name='Methane oxidation (Fe)',stoich='1.0 CH4(aq)  + 8.0 Fe+++ + 3.0 H2O -> 1.0 HCO3-  + 8.0 Fe++ + 9.0 H+ + 1.0 Tracer ',
                                            monod_terms=[decomp_network.monod(species='Fe+++',k=conc_scales['Fe+++'],threshold=1.1e-12),decomp_network.monod(species='CH4(aq)',k=conc_scales['CH4(aq)'],threshold=1.1e-16)],
                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
...
simlength=30
result,units=decomp_network.PF_network_writer(reaction_network).run_simulation('SOMdecomp_template.txt','saline',pflotran_exe,print_output=False,length_days=simlength,truncate_concentration=truncate_conc)
