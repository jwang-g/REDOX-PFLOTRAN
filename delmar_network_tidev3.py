...
decomp_network.decomp_pool(name='O2(aq)',kind='primary',constraints={'initial':'0.2 G O2(g)','sed_air_interface':1e-3,'excretion':1e-20,'tide':1e-2,'sed_water_interface':1e-5}),
...
#decomp_network.decomp_pool(name='FeS(aq)',kind='secondary'),  # cjw add new secondary species for Pyrite
#decomp_network.decomp_pool(name='FeS2(aq)',kind='secondary'),  #cjw add new secondary gas_species
#decomp_network.decomp_pool(name='S--',kind='secondary'),


decomp_network.decomp_pool(name='Fe(OH)3',rate='1.d-8 mol/m^2-sec',constraints={'initial':'0.1d-3  1.d2 m^2/m^3'},kind='mineral'),
decomp_network.decomp_pool(name='Fe(OH)2',rate='1.d-7 mol/m^2-sec',constraints={'initial':'0.0e-20  1.d2 m^2/m^3'},kind='mineral'),
decomp_network.decomp_pool(name='Pyrite',rate='0.d-8 mol/m^2-sec',constraints={'initial':'0.1d-3  1.d2 m^2/m^3'},kind='mineral'),   # 8.2e-8 to 1.2e-7 mol/m2/s after Oene Oenema, 1990, Pyrite accumulation in slat marshes in the Eastern Scheldt southwest Netherlands
#decomp_network.decomp_pool(name='Pyrite1',rate='0.d-6 mol/m^2-sec',constraints={'initial':'0.1d-3  1.d2 m^2/m^3'},kind='mineral'),
decomp_network.decomp_pool(name='Calcite',rate='0.d-3 mol/m^2-sec',constraints={'initial':'0.875d-3  1.d2 m^2/m^3'},kind='mineral'),
decomp_network.decomp_pool(name='Pyrrhotite',rate='1.d-9 mol/m^2-sec',constraints={'initial':'0.1d-3  1.d2 m^2/m^3'},kind='mineral'),    #works without pyrite, because pyrite include SO4-- + HS-, pyrite1 include SO4-- and produce O2
...
def make_network(change_constraints={},change_rate={}):
    conc_scales={
    	'DOM1':1e-2,
    	'Acetate-':1e-3,
    	'Fe+++':1e-7,   #1e-10 will have negative Fe+++ for both; 1e-8 will have negetative for fresh
        'Fe++':1e-3,  #1e-4, 1e-1 works with Pyrrhotite precipiration rate 1.d-11 and 1.d-9
    	'NH4+':1e-5,
    	'HCO3-':1e-2,
    	'NO3-':1e-6,
        'N2O(aq)':1e-10,
    	'SO4--':1e-3,#1e-3,     #change here to test methane flux
        'HS-':1e-3,   #1e-4 works with Fe++ 1e-1 and also works with Pyrrhotite precipiration rate 1.d-11 and 1.d-9
    	'CH4(aq)':1e-4,  #1e-4 also work without pyrite precipitation
    	'H2(aq)':1e-4,
    	'O2(aq)':1e-4,
    }
    rate_scale=2e-10
...
#	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
        # FeS2 formation in porewater to precipitate pirite.
        #decomp_network.reaction(name='Fe(II) Sulfide formation',stoich='1.0 Fe++ + 1.0 HS- -> 1.0 FeS + 1.0 H+',
        #                                        monod_terms=[decomp_network.monod(species='O2(aq)',k=conc_scales['O2(aq)'],threshold=thresh),decomp_network.monod(species='Fe++',k=conc_scales['Fe++'],threshold=thresh)],
        #                                        rate_constant=1e-8,reactiontype='MICROBIAL'),
...
#	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
        ##without inhibition of Fe, NO3,SO4
        decomp_network.reaction(name='Acetoclastic methanogenesis',stoich='1.0 Acetate- + 1.00e+00 H2O -> 1.0 CH4(aq) + 1.0 HCO3- + 1.0 Tracer',
	                                            monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=thresh),],
	                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
	                                            #decomp_network.inhibition(species='Fe+++',k=conc_scales['Fe+++'],type='MONOD'),
	                                            #decomp_network.inhibition(species='NO3-',k=conc_scales['NO3-'],type='MONOD'),
...
#	                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
#	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
#cjw update stoic to balance the reaction, N2O is ignored here. (N2O is produced when electron is not enough)
	    # Denitrification C2H3O2- + 2 NO3- + H+ -> 2 HCO3- + N2 + 2 H2O
        #decomp_network.reaction(name='Denitrification',stoich='1.25 Acetate- + 2.0 NO3- + 0.75 H+ -> 2.5 HCO3- + 1.0 N2(aq) + 1.0 H2O + 2.5 Tracer',
	    #                                        monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=thresh),decomp_network.monod(species='NO3-',k=conc_scales['NO3-'],threshold=thresh)],
	    #                                        inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
	    #                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
        decomp_network.reaction(name='Denitrification1',stoich='1.0 Acetate- + 2.0 NO3- + 1.0 H+ -> 2.0 HCO3- + 1.0 N2O(aq) + 1.0 H2O + 2.0 Tracer',
	                                            monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=thresh),decomp_network.monod(species='NO3-',k=conc_scales['NO3-'],threshold=thresh)],
	                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
        decomp_network.reaction(name='Denitrification2',stoich='0.25 Acetate- + 1.0 N2O(aq) + 1.0 H+ -> 0.5 HCO3- + 1.0 N2(aq) + 0.25 H+ + 0.5 Tracer',
	                                            monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=thresh),decomp_network.monod(species='N2O(aq)',k=conc_scales['N2O(aq)'],threshold=thresh)],
	                                            inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD'),decomp_network.inhibition(species='NO3-',k=conc_scales['NO3-'],type='MONOD')],
	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
        #decomp_network.reaction(name='Denitrification',stoich='2.25 Acetate- + 4.0 NO3- + 1.75 H+ -> 4.5 HCO3- + 1.0 N2(aq) + 1.0 N2O(aq) + 2.0 H2O + 4.5 Tracer',
	    #                                        monod_terms=[decomp_network.monod(species='Acetate-',k=conc_scales['Acetate-'],threshold=thresh),decomp_network.monod(species='NO3-',k=conc_scales['NO3-'],threshold=thresh)],
	    #                                        inhibition_terms=[decomp_network.inhibition(species='O2(aq)',k=conc_scales['O2(aq)'],type='MONOD')],
	    #                                        rate_constant=rate_scale,reactiontype='MICROBIAL'),
        ##added by Jiaze MMMM
	    ...
	    decomp_network.reaction(name='Autotrophic DNRA',stoich='1.0 HS- + 1.0 NO3- + 1.0 H+ + 1.0 H2O -> 1.0 SO4-- + 1.0 NH4+',
                                                ...
                                                inhibition_terms=[decomp_network.inhibition(species='DOM1',type='MONOD',k=conc_scales['DOM1']),decomp_network.inhibition(species='Fe++',k=conc_scales['Fe++'],type='MONOD')],
	                                            rate_constant=rate_scale,reactiontype='MICROBIAL'),
        ##added by Jiaze wwww
	    ...
	    decomp_network.reaction(name='Methane oxidation (O2)',stoich='1.0 CH4(aq)  + 2.0 O2(aq)  -> 1.0 HCO3-  + 1.0 H+ + 1.0 H2O + 1.0 Tracer',
	    ...
	    #decomp_network.reaction(name='Methane oxidation (NO3)',stoich='1.0 CH4(aq)  + 1.0 NO3- + 1.0 H+ -> 1.0 HCO3-  + 1.0 NH4+ + 1.0 Tracer ',
        decomp_network.reaction(name='Methane oxidation (NO3)',stoich='5.0 CH4(aq)  + 8.0 NO3- + 3.00e+00 H+ -> 5.0 HCO3-  + 4.0 N2(aq) + 9.00e+00 H2O + 1.0 Tracer ',
