...
import deltamarsh_network as Mar       #deltamarsh_network is methanogenesis without inhibition
...
#for station in stnm:
nm=['CRMS3166','CRMS2825']#['CRMS0220','CRMS4245','CRMS6304','CRMS0178','CRMS0307','CRMS0464','CRMS0463','CRMS0224','CRMS0175','CRMS0164','CRMS3169','CRMS5373','CRMS0479']#['CRMS0164','CRMS0175','CRMS0190','CRMS0396','CRMS0192']  #rerun 0164,0479,0307,0192,0175,0396 (,'CRMS0479','CRMS0307') CRMS6304,464,307,463,224,479,178 is negative with HS-
...
##cjw parameter needed for transport
    ...
    wtl=pd.read_csv('/Users/46w/Documents/estuary wetland/CRMS_data/'+hyfile1)         ##
    salinity=pd.read_csv('/Users/46w/Documents/estuary wetland/CRMS_data/'+hyfile2)
    wtemp=pd.read_csv('/Users/46w/Documents/estuary wetland/CRMS_data/'+hyfile3)
    wtemp=wtemp.interpolate()
    wnd=pd.read_csv('/Users/46w/Documents/estuary wetland/wind/'+hyfile4)
...
#    Tw=[x for x in Tw[24*365*2:24*(365*4+366)-1]]
    ...
    MSL=0.1      ## elevated mean sea level in meters after 20 yrs with 10mm/yr rising rate; 10 yrs with 10mm/yr
    dsal=5.0       ## elevated salinity increase or elevated salt water intrusion
...
#    sulfc=salfc/0.14 ## sulfate concetration is mg/L, and 1e-3*mg/L/molar_mass=M
    ...
    for spec in tconc_fc.keys():
        ...
        #    tmp=[i*0.0+0.00068 for i in sf]
            ...
            tmp=[i*0.0+0.022*1e-6 for i in sf]          #0.022 uM is from Brooks, 1975; 7.48*1e-6 is from Songjie He et al., water measurement in Barataria Bay.
        #elif spec=='H2(aq)':
...
#cjw not sure what is in output
    ...
    fname='./3V3166Results/floodsal_temp_Methane_output_{year:04d}-{month:02d}-{day:02d}.nc'.format(year=today.year,month=today.month,day=today.day)
    fname=fname[:-3]+ebp+stnm[4:8]+'.nc'
    Lname='./3V3166Results/floodsal_temp_LtranR_{year:04d}-{month:02d}-{day:02d}.npy'.format(year=today.year,month=today.month,day=today.day)
    Lname=Lname[:-4]+ebp+stnm[4:8]+'.npy'
    Jname='./3V3166Results/floodsal_temp_Jswi_{year:04d}-{month:02d}-{day:02d}.npy'.format(year=today.year,month=today.month,day=today.day)
    Jname=Jname[:-4]+ebp+stnm[4:8]+'.npy'
    Ename='./3V3166Results/floodsal_temp_Ebull_{year:04d}-{month:02d}-{day:02d}.npy'.format(year=today.year,month=today.month,day=today.day)
